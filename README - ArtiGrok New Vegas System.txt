...
This damage value, which is initialized from the Starter Damage, takes these components into account:
	...
	- AmmoDT_mult = 0.80 (Grok: last multiplier is a new adjustment after the bone grouping update)
...
NOTE: GAMMA has ammo's k_ap values divided by 10 to reduce multi-hit situtations, so know that when we use k_ap here, it has already been multipled by 10 to get its true value back.
	...
	k_ap = k_ap * falloff_modifier * isg_res * silencer_boost * ((is_actor and difficulty_multiplier[game_num]) or npc_difficulty_multiplier[game_num]) * 0.80
...
These will be the components that will make up the calculations for our final value:
	- LM (locational damage) = (custom_bone_dmg_mult * 1.1) & sniper_bad_bone_shit_reduce (base GAMMA values)
...
Using the new remaining DT that we calculated earlier, we will determine what to do with our damage, along with other systems pertaining to that.
If there is still new remaining DT, we apply the penalty modifier.

		final_base_damage = final_base_damage * hp_penality_modifier
...
In order to balance around GAMMA's armor values, I have decided to make a personal design choice regarding DT and penetration.
If the new remaining DT is 5 points (0.05) or LESS, the hit will count as penetrative, meaning the enemy's DT has been broken.
This is to make it more forgiving in case an ammo type is a few points away from breaking DT since we no longer subtract armor on hits.

Having said that, if an enemy had the DT bypassed and destroyed on the hit limb, there are a few things that happen with that:
	...
	(2) That hit limb will have 0 DT for 5 seconds. After that, DT is back to its normal value.
		...
		if new_remaining_threshold <= 0.05 then
			...
			-- Ensures that if enemy had another limb's DT broken, it doesn't immediately take it away when the first limb's DT break is up
	...
	(4) If the new remaining DT is 5 points (0.05) or less AND the final damage is less than 20% of the Resist Damage from earlier, it will be bumped up to that 20%.
		This is in the event that the ammo's HP penality kicks and reduces the damage too much on a "pen. hit".
		
		------------------------------------------------------------------------------------------------------------
		--- NOTE: This might be changed in the future as this might be really redundant or outright unnecessary. ---
		------------------------------------------------------------------------------------------------------------
	
		if new_remaining_threshold <= 0.05 then
			if final_base_damage < (resist_damage * 0.2) then
				final_base_damage = resist_damage * 0.2
			end
		end
...
Overall, it should make fights easier or harder depending on the ammo you are using in each situation. Some variant ammo might even get to shine more as you get more opporutnities to
stack their effects.
